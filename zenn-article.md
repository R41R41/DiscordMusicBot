...
YouTube 配信をするとき、Discord の VC に自分や他の出演者が集まって話しながら配信しています。
そこで **BGM を流したかった**。音楽があった方がテンション上がるし、配信の雰囲気も良くなる。

OBS で BGM を流す方法もありますが、それだと **視聴者と自分にしか聴こえない**。一緒に VC にいる出演者には届かないので、配信の雰囲気が共有できない。全員が同じ BGM を聴きながら喋れた方がいい。

じゃあ Discord の VC に Bot で流せばいいじゃん、と思ったものの、既存の手段にはこんな問題がありました：

- **画面共有 + OBS で VC に流す**: 音質が配信品質に依存する。BGM を流したいだけなのに大げさ
- **既存の音楽Bot（Rythm系）**: YouTube 等のストリーミング前提で、ローカルの自作曲を流せない
- **ファイルを直接送る**: VC でリアルタイムに流せない。「今この曲聴いて！」ができない

**「ローカルの音楽ファイルを、Bot が直接 VC に流してくれて、Web UI で操作できる」** というものが欲しかった。
それなら自分で作るか、と。
